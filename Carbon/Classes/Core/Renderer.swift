...
open class Renderer<Updater: Carbon.Updater> {
    ...
    open func render<C: Collection>(_ data: C) where C.Element == CarbonUnit {
        var result: [Section] = []
        var section: Section?
        ...
        for (index, body) in data.enumerated() {
            switch body {
//            case let body as Section:
//                close()
//                result.append(body)
                
            case let body as SectionsBuildable:
                close()
                result.append(contentsOf: body.buildSections())
                
                
            case let body as CellsBuildable:
                let cells = body.buildCells()
                section = section ?? Section(id: UniqueIdentifier())
